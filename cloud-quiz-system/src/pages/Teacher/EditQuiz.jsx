...
import { 
  getQuizById, getQuizQuestions, updateQuiz, deleteQuiz,
  addQuestionToQuiz, updateQuestion, deleteQuestion 
...
export default function EditQuiz() {
  ...
  const [deadline, setDeadline] = useState(""); // ISO date string for datetime-local input
  ...
  const loadQuiz = async () => {
    try {
      ...
      setDeadline(quizData.deadline ? new Date(quizData.deadline).toISOString().slice(0, 16) : '');
        ...
        <div className="card">
          ...
          <form onSubmit={handleUpdateQuiz}>
            ...
            <div style={{ marginBottom: '8px' }}>
              ...
              />
              {deadline && (
                <div style={{ marginTop: '6px', fontSize: '12px', color: '#64748b', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>Closes: {new Date(deadline).toLocaleString()}</span>
                  <button type="button" onClick={() => setDeadline('')} style={{ background: 'transparent', color: '#ef4444', fontSize: '12px', padding: '0', border: 'none', cursor: 'pointer', fontWeight: '600' }}>✕ Remove</button>
                </div>
              )}
            </div>
