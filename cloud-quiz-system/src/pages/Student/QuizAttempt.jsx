...
export default function QuizAttempt() {
  ...
  const [adaptiveStats, setAdaptiveStats] = useState({ correct: 0, total: 0, currentDifficulty: 'medium' });
  ...
  useEffect(() => {
    const loadQuiz = async () => {
      try {
        ...
        if (quiz?.deadline && Date.now() > quiz.deadline) {
          error('This quiz has expired and is no longer available.', 5000);
          navigate('/student');
          return;
        }
  ...
  const handleSubmit = async () => {
    ...
    success(`Quiz submitted! Score: ${score}/${questions.length}`, 3000);
    // Navigate to result page with full data for AI Tutor
    ...
    <>
      ...
      <div className="dashboard-container">
        ...
        <div className="quiz-info">
          ...
          {quizInfo.deadline && (
            <div className="info-card" style={{ borderTop: `3px solid ${Date.now() > quizInfo.deadline - 3600000 ? '#ef4444' : '#f59e0b'}` }}>
              <h4>Deadline</h4>
              <p style={{ fontSize: '13px' }}>{new Date(quizInfo.deadline).toLocaleString()}</p>
            </div>
          )}
        </div>
        ...
        <div className="card">
          ...
          <div style={{ marginBottom: '12px' }}>
            <div className="ai-status">
              {aiMonitoringActive ? (
                <span className="ai-status-active">🤖 AI Monitoring: Active</span>
              ) : (
                <span className="ai-status-loading">🤖 AI Monitoring: Loading...</span>
              )}
            </div>
              ...
              {q.options.map((opt, i) => (
                <div 
                  ...
                  onClick={() => {
                    ...
                    // Track adaptive stats when answer changes
                    const wasCorrect = i === Number(q.correctIndex);
                    setAdaptiveStats(prev => ({
                      correct: prev.correct + (wasCorrect ? 1 : 0),
                      total: prev.total + 1,
                      currentDifficulty: q.difficulty || prev.currentDifficulty,
                    }));
                  }}
                >
                  <input
